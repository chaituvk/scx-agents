...
// Static KB articles that are always available
const STATIC_KB: KnowledgeDoc[] = [
 ...
 */
export async function retrieveKnowledge(userQuery: string, topK: number = 3): Promise<KnowledgeDoc[]> {
  const keywords = userQuery.toLowerCase().split(/\s+/).filter((w) => w.length > 2);
  const results: KnowledgeDoc[] = [];

  // ── Layer 1: Dynamic knowledge_sources from DB ──────────────────────
  try {
    const dbResult = await query("SELECT * FROM knowledge_sources WHERE status = 'active'");
    ...
    // DB unavailable — fall back to static KB only
  }

  // ── Layer 2: Static KB articles ─────────────────────────────────────
  for (const doc of STATIC_KB) {
  ...
  // If no keywords matched, do a fuzzy substring search on static KB
  if (results.length === 0) {
